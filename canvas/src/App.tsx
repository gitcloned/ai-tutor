...
const modelShapeUtils=[ModelShapeUtil];
...
export default function App() {
  ...
  const [url,setUrl]=useState(()=>localStorage.getItem('prodigy-ws')||'ws://localhost:32004');
  ...
  async function sendWork(audio?:MediaInput){
    ...
    if(sending.current||!work.current||!editor)return;
    sending.current=true;const version=lesson.connectionVersion.current,page=editor.getCurrentPageId(),draft=replyRef.current.trim(),voice=pendingAudio.current;
    try{
      editor.complete();
    ...
    }catch{lesson.notify('Could not prepare your work. It is still pending; tap to retry.');}finally{sending.current=false;}
  }
  ...
  return <main className="app">
    ...
    <section className="canvas-area" aria-label="Shared learning canvas" onPointerDown={e=>{canvasPointer.current=(e.target as HTMLElement).closest('.tl-canvas')?{x:e.clientX,y:e.clientY}:null;}} onPointerMove={e=>{const start=canvasPointer.current;if(start&&e.buttons&&Math.hypot(e.clientX-start.x,e.clientY-start.y)>8){lesson.stopFollowing();canvasPointer.current=null;}}} onPointerUp={()=>{canvasPointer.current=null;}} onPointerCancel={()=>{canvasPointer.current=null;}} onWheel={()=>lesson.stopFollowing()}>
      <Tldraw hideUi shapeUtils={modelShapeUtils} persistenceKey="prodigy-canvas-v1" onMount={ed=>{applyCanvasTheme(ed);ed.user.updateUserPreferences({colorScheme:'light'});ed.updateInstanceState({isGridMode:true});setEditor(ed);}}><Toolbar/></Tldraw>
    </section>
    ...
    <Orb phase={lesson.phase} onTap={tapOrb} onAudio={audio=>{void sendWork(audio);}} onRecording={lesson.recording} notify={lesson.notify}/>
