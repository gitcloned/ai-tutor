...
export type Block = { kind: 'session' | 'model3d' | 'write' | 'svg' | 'ask' | 'play' | 'speech' | 'audio' | 'error' | 'action'; content: string; attrs: Attrs; action?: unknown };
export class BlockAdapter {
  ...
  accept(event: WireEvent): Block[] {
    ...
    if (['model3d','svg','ask','play','audio_chunk','error','action'].includes(event.type)) flush();
    const attrs = event.attrs ?? {};
    if (['model3d','svg','ask','play'].includes(event.type)) result.push({kind:event.type as 'model3d'|'svg'|'ask'|'play',content:event.content ?? '',attrs});
