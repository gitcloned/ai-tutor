...
const root = resolve(fileURLToPath(new URL('./3d-models/', import.meta.url)));
export const resourceServer = () => createServer(async (req, res) => {
  ...
  try {
    ...
    if (!url.pathname.startsWith('/3d-models/')) throw new Error('Not found');
    const path = resolve(root, decodeURIComponent(url.pathname.slice('/3d-models/'.length)));
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  resourceServer().listen(port, '127.0.0.1', () => console.log(`3D resources: http://127.0.0.1:${port}/3d-models/catalog.json`));
}
