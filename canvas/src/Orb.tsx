...
export function Orb({tutorBusy=false,phase,submission='idle',preparing=false,watching=false,onCamera,pageCount=0,onTap,onAudio,onRecording,notify}:{tutorBusy?:boolean;phase:Phase;submission?:'idle'|'sent'|'waiting';preparing?:boolean;watching?:boolean;onCamera?:()=>void;pageCount?:number;onTap:()=>void;onAudio:(audio:MediaInput)=>void;onRecording:(active:boolean)=>void;notify:(text:string,action?:'connect'|'reply'|'retry'|'sound')=>void}) {
  ...
  const tapCallbacks=useRef({onTap,onCamera,unavailable:false});
  ...
  const unavailable=phase!=='offline'&&phase!=='connecting'&&(tutorBusy||preparing||submission!=='idle'||watching);
  tapCallbacks.current={onTap,onCamera,unavailable};
  useEffect(()=>{if(unavailable&&tapTimer.current){clearTimeout(tapTimer.current);tapTimer.current=null;}},[unavailable]);
  ...
  useEffect(()=>()=>{if(tapTimer.current)clearTimeout(tapTimer.current);generation.current++;cancelled.current=true;pressed.current=false;if(timer.current)clearTimeout(timer.current);if(recorder.current?.state==='recording')recorder.current.stop();stopTracks();callbacks.current.onRecording(false);},[]);
  ...
  function begin(){if(unavailable||pressed.current||recorder.current)return;openCameraClick.current=false;secondTap.current=!!tapTimer.current;if(tapTimer.current){clearTimeout(tapTimer.current);tapTimer.current=null;}pressed.current=true;setPressing(true);held.current=false;cancelled.current=false;timer.current=setTimeout(start,420);}
  function release(cancel=false,keyboard=false){
    if(!pressed.current)return;const wasHeld=held.current;stop(cancel);
    if(cancel||wasHeld||unavailable)return;
    if(keyboard||!onCamera||phase==='offline'||phase==='connecting'){onTap();return;}
    // Open on the completed click: mounting a dialog on pointerup can retarget
    // the touch's compatibility click to its newly visible Send button.
    if(secondTap.current){secondTap.current=false;openCameraClick.current=true;}
  ...
  return <div className="orb-wrap">
    ...
    <button data-state={state} className={`orb ${busy?'active':''} ${listening?'listening':''} ${unavailable?'unavailable':'available'} ${submission==='sent'?'submitted':''}`} disabled={unavailable} aria-busy={tutorBusy||preparing||submission==='waiting'} aria-label={listening?'Release to send audio':'Send new work; hold to speak'}
      ...
      onClick={()=>{if(openCameraClick.current&&!unavailable){openCameraClick.current=false;onCamera?.();}}}
      onTouchEnd={e=>{
        // Safari may omit the compatibility click after a prevented pointerdown.
        // Consume the native touch end so it cannot click through the new dialog.
        if(e.touches.length===0){release(cancelled.current);if(openCameraClick.current&&!unavailable){e.preventDefault();openCameraClick.current=false;onCamera?.();}}
      }}
      ...
      onKeyUp={e=>{if(e.key===' '||e.key==='Enter'){e.preventDefault();release(false,true);}}}>
      {pageCount>0&&<span className="orb-page-count" aria-label={`${pageCount} pages attached`}>{pageCount}</span>}
    ...
    <span className="orb-hint">{listening?'Release to send · Slide away to cancel':submission==='sent'?'Your work is on its way':unavailable?'You can keep working on the canvas':phase==='offline'?'Tap to connect':pageCount?`${pageCount} pages · Tap to send`:'Tap to send · Hold to talk · Double-tap for camera'}</span>
  </div>;
}
